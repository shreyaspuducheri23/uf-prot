...
def pearson_r_matrix(snp_list: list[str], bfile: Path = LD_REF_PREFIX) -> pd.DataFrame:
    """
    Compute pairwise signed Pearson r matrix for snp_list via plink2 --r-unphased square.
    ...
    with tempfile.TemporaryDirectory(prefix="ld_matrix_") as tmpdir:
        ...
        cmd = [
            ...
            "--r-unphased", "square",        # their correlation is undefined (NaN) and unreliable
            "--out", str(out_prefix),
        ]
        _run(cmd)
        ...
        if not matrix_file.exists():
            raise FileNotFoundError(f"plink2 Pearson r output not found: {matrix_file}")
        ...
        if nan_rows.any():
            log.debug("pearson_r_matrix: dropping %d SNPs with residual NaN LD entries",
                      nan_rows.sum())
