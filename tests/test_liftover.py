...
class TestLiftPosition:
    def test_known_snp_exact_mapping(self):
        # rs4988235 (chr2:136608646 hg19) -> chr2:135851076 hg38
        result = lift_position("2", 136_608_646)
        assert result is not None
        chrom, pos = result
        assert chrom == "2"
        assert pos == 135_851_076

    def test_impossible_chromosome_returns_none(self):
        assert lift_position("999", 1000) is None
...
class TestLiftTable:
    ...
    def test_drops_failed_liftover(self, tmp_path):
        df = pd.DataFrame({"chrom": ["999"], "pos": [1_000]})
        result = lift_table(df)
    ...
    def test_drop_rate_logged(self, caplog):
        ...
        with caplog.at_level(logging.WARNING, logger="scripts.lib.liftover"):
            result = lift_table(df)
